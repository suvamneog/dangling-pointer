...
extension Notification.Name {
    static let danglingPointerDismissPanel = Notification.Name("danglingPointerDismissPanel")
}
...
final class MenuBarPanelManager: NSObject {
    ...
    init(companionManager: CompanionManager) {
        ...
        dismissPanelObserver = NotificationCenter.default.addObserver(
            forName: .danglingPointerDismissPanel,
            object: nil,
            queue: .main
        ) { [weak self] _ in
            self?.hidePanel()
    ...
    /// Patrick cursor icon for the menu bar (matches the on-screen buddy).
    private func makeMenuBarIcon() -> NSImage {
        let iconSize = NSSize(width: 18, height: 18)
        if let pngURL = Bundle.main.url(forResource: "patrick-cursor", withExtension: "png"),
           let sourceImage = NSImage(contentsOf: pngURL) {
        ...
        // Fallback: small pointing triangle if the PNG is missing.
        ...
        path.move(to: CGPoint(x: cx, y: cy + height / 1.5))
        path.line(to: CGPoint(x: cx - triangleSize / 2, y: cy - height / 3))
        path.line(to: CGPoint(x: cx + triangleSize / 2, y: cy - height / 3))
        path.close()
