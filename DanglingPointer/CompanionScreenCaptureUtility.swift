...
struct CompanionScreenCapture {
    ...
    /// Maps a point the model gave in a normalized grid over this screenshot
    /// (top-left origin, 0...normalizedSpaceSize on both axes) to AppKit
    /// global coordinates (bottom-left origin) on this display.
    func globalScreenLocation(forNormalizedPoint normalizedPoint: CGPoint, normalizedSpaceSize: CGFloat) -> CGPoint {
        let clampedX = max(0, min(normalizedPoint.x, normalizedSpaceSize))
        let clampedY = max(0, min(normalizedPoint.y, normalizedSpaceSize))

        let displayLocalX = clampedX * (CGFloat(displayWidthInPoints) / normalizedSpaceSize)
        let displayLocalY = clampedY * (CGFloat(displayHeightInPoints) / normalizedSpaceSize)
        let appKitY = CGFloat(displayHeightInPoints) - displayLocalY
...
enum CompanionScreenCaptureUtility {
    ...
    /// `onlyCursorScreen` skips the other displays, and `maxDimension` /
    /// `compressionFactor` shrink the image when the consumer counts tokens.
    static func captureAllScreensAsJPEG(
        onlyCursorScreen: Bool = false,
        maxDimension: Int = 1920,
        compressionFactor: CGFloat = 0.8
        ...
        for (displayIndex, display) in sortedDisplays.enumerated() {
            ...
            if onlyCursorScreen && !isCursorScreen {
                continue
            }
