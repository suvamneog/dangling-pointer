...
// Cursor-like triangle shape (kept for any leftover references).
...
/// Buddy art from the app bundle (Patrick cursor / pointer PNGs).
private struct BuddyAssetView: View {
    var resourceName: String

    var body: some View {
        Group {
            if let buddyImage = Self.loadBuddyImage(named: resourceName) {
                Image(nsImage: buddyImage)
                    .resizable()
                    .interpolation(.high)
    ...
    private static func loadBuddyImage(named resourceName: String) -> NSImage? {
        for fileExtension in ["png", "gif"] {
            if let assetURL = Bundle.main.url(forResource: resourceName, withExtension: fileExtension),
               let buddyImage = NSImage(contentsOf: assetURL) {
                return buddyImage
            }
        }
        return nil
...
struct BlueCursorView: View {
    ...
    private let fullWelcomeMessage = "hey! i'm dangling pointer"

    /// True when this overlay sits on the screen that owns the menu bar / notch.
    private var isMenuBarScreen: Bool {
        guard let menuBarScreen = NSScreen.main else { return false }
        return menuBarScreen.frame == screenFrame
    }

    /// Sit just under the notch / menu bar. Uses safeAreaInsets when present.
    private var islandTopInset: CGFloat {
        guard let screen = NSScreen.screens.first(where: { $0.frame == screenFrame }) else {
            return 28
        }
        let notchInset = screen.safeAreaInsets.top
        return notchInset > 0 ? notchInset + 2 : 28
    }
    ...
    var body: some View {
        ZStack {
            ...
            // Dynamic Island — only on the menu-bar / notch screen so we don't
            // duplicate a pill on every monitor.
            if isMenuBarScreen {
                DynamicIslandView(companionManager: companionManager)
            ...
            // Onboarding prompt — "press control + option…" after welcome (no video)
            ...
            // Pink arrow while following; lip-bite face while pointing or clicking.
            let isClickBurst = companionManager.isPlayingDeathBeamBurst
            let isPointingBuddy = !isClickBurst && buddyNavigationMode != .followingCursor
            let buddyResource = (isClickBurst || isPointingBuddy) ? "patrick-pointer" : "patrick-cursor"
            let buddyWidth: CGFloat = isClickBurst ? 48 : (isPointingBuddy ? 36 : 24)
            let buddyHeight: CGFloat = isClickBurst ? 52 : (isPointingBuddy ? 40 : 36)

            BuddyAssetView(resourceName: buddyResource)
                .id(buddyResource)
                .frame(width: buddyWidth, height: buddyHeight)
                .shadow(
                    color: Color.pink.opacity(isClickBurst ? 0.9 : 0.5),
                    radius: isClickBurst ? 14 : (4 + (buddyFlightScale - 1.0) * 8),
                    x: 0,
                    y: 0
                )
                .shadow(
                    color: Color.white.opacity(isClickBurst ? 0.45 : 0),
                    radius: isClickBurst ? 8 : 0,
                    x: 0,
                    y: 0
                )
                .scaleEffect(buddyFlightScale * (isClickBurst ? 1.25 : 1.0))
                .opacity(buddyIsVisibleOnThisScreen && (companionManager.voiceState == .idle || companionManager.voiceState == .responding || isClickBurst) ? cursorOpacity : 0)
                ...
                .animation(.spring(response: 0.28, dampingFraction: 0.72), value: isClickBurst)
                .animation(.easeIn(duration: 0.25), value: companionManager.voiceState)
                .animation(.easeInOut(duration: 0.2), value: isPointingBuddy)
                .animation(.easeInOut(duration: 0.15), value: buddyResource)
        ...
        .onAppear {
            ...
            // Tip of the pink arrow sits on the real mouse hotspot.
            self.cursorPosition = CGPoint(x: swiftUIPosition.x + 12, y: swiftUIPosition.y + 18)

            startTrackingCursor()
            syncSystemCursorHiddenWithBuddy()
    ...
    private func startTrackingCursor() {
        timer = Timer.scheduledTimer(withTimeInterval: 0.016, repeats: true) { _ in
            ...
            // Normal cursor following — Patrick tip tracks the mouse hotspot.
            let swiftUIPosition = self.convertScreenPointToSwiftUICoordinates(mouseLocation)
            let buddyX = swiftUIPosition.x + 12
            let buddyY = swiftUIPosition.y + 18
            self.cursorPosition = CGPoint(x: buddyX, y: buddyY)
            self.syncSystemCursorHiddenWithBuddy()
        }
    }

    /// Hide the macOS arrow while Patrick is the visible pointer on this screen.
    /// NSCursor hide/unhide is reference-counted — only toggle when state changes.
    private func syncSystemCursorHiddenWithBuddy() {
        let buddyIsDrawn = buddyIsVisibleOnThisScreen
            && (companionManager.voiceState == .idle
                || companionManager.voiceState == .responding
                || companionManager.isPlayingDeathBeamBurst)
            && cursorOpacity > 0.01

        if buddyIsDrawn && !isSystemCursorHidden {
            NSCursor.hide()
            isSystemCursorHidden = true
        } else if !buddyIsDrawn && isSystemCursorHidden {
            NSCursor.unhide()
        ...
        Timer.scheduledTimer(withTimeInterval: 0.03, repeats: true) { timer in
            guard currentIndex < self.fullWelcomeMessage.count else {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 2.5) {
                    ...
                    // Short tip after welcome — no intro video.
                    self.companionManager.startOnboardingPrompt()
                }
                return
            }
